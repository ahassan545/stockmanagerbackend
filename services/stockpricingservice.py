...
class StockPricingService:
    ...
    def _get_data(self, ticker: str, date: datetime, days: int = 90):
        ...
        result["return_10d"] = data["Close"].pct_change(10).fillna(0).to_numpy().flatten()
        result["volatility_5d"] = result["return_1d"].rolling(5).std().fillna(0).to_numpy().flatten()
        result["volatility_10d"] = result["return_1d"].rolling(10).std().fillna(0).to_numpy().flatten()
        result["return_1d"] = result["return_1d"].fillna(0).to_numpy().flatten()
        df = pd.DataFrame(result)       

        return df[["return_1d", "return_5d", "return_10d", "volatility_5d", "volatility_10d"]], data[["Close"]].to_numpy().flatten()
